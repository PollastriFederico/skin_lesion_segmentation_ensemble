...
def _AsppConv(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bn_momentum=0.1, bn_eps=1e-05):
    asppconv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, bias=False),
            SynchronizedBatchNorm2d(out_channels, momentum=bn_momentum, eps=bn_eps),
            nn.ReLU()
        )
    return asppconv
...
class AsppModule(nn.Module):
    def __init__(self, bn_momentum=0.1, output_stride=16, bn_eps=1e-05):
        ...
        self._atrous_convolution1 = _AsppConv(2048, 256, 1, 1, bn_momentum=bn_momentum, bn_eps=bn_eps)
...
class Encoder(nn.Module):
    def __init__(self, bn_momentum=0.1, output_stride=16, bn_eps=1e-05):
        ...
        self.ASPP = AsppModule(bn_momentum=bn_momentum, output_stride=output_stride, bn_eps=bn_eps)
        self.relu = nn.ReLU()
        self.conv1 = nn.Conv2d(1280, 256, 1, bias=False)
        self.bn1 = SynchronizedBatchNorm2d(256, momentum=bn_momentum, eps=bn_eps)
