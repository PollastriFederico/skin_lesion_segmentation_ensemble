...
class Decoder(nn.Module):
    def __init__(self, class_num, bn_momentum=0.1, bn_eps=1e-05):
        super(Decoder, self).__init__()
        self.conv1 = nn.Conv2d(256, 48, kernel_size=1, bias=False)
        self.bn1 = SynchronizedBatchNorm2d(48, momentum=bn_momentum, eps=bn_eps)
        self.relu = nn.ReLU()
        ...
        self.bn2 = SynchronizedBatchNorm2d(256, momentum=bn_momentum, eps=bn_eps)
        self.dropout2 = nn.Dropout(0.5)
        self.conv3 = nn.Conv2d(256, 256, kernel_size=3, padding=1, bias=False)
        self.bn3 = SynchronizedBatchNorm2d(256, momentum=bn_momentum, eps=bn_eps)
...
class DeepLab(nn.Module):
    def __init__(self, output_stride, class_num, num_input_channel, pretrained, bn_eps=1e-05, bn_momentum=0.1, freeze_bn=False):
        super(DeepLab, self).__init__()
        self.Resnet101 = resnet101(bn_momentum, pretrained, num_input_channel, bn_eps=bn_eps)
        self.encoder = Encoder(bn_momentum, output_stride, bn_eps)
        self.decoder = Decoder(class_num, bn_momentum, bn_eps)
        if freeze_bn:
            self.freeze_bn()
