...
from .SplitResnet101 import first_resnet101, second_resnet101
...
class FeatureExtractor(nn.Module):
    def __init__(self, pretrained=True, bn_momentum=0.1, freeze_bn=False):
        super(FeatureExtractor, self).__init__()
        self.bn_momentum = bn_momentum
        self.FirstResnet101 = first_resnet101(bn_momentum, pretrained)
        if freeze_bn:
            self.freeze_bn()
            print("freeze bacth normalization successfully!")

    def forward(self, input):
        low_level_features = self.FirstResnet101(input)
...
class FinalLayers(nn.Module):
    def __init__(self, class_num, output_stride=16, pretrained=True, bn_momentum=0.1, freeze_bn=False):
        ...
        self.bn_momentum = bn_momentum
        self.SecondResnet101 = second_resnet101(bn_momentum, pretrained)
    ...
    def forward(self, low_level_features):
        x = self.SecondResnet101(low_level_features)
        x = self.encoder(x)
        predict = self.decoder(x, low_level_features)
        output = F.interpolate(predict, size=low_level_features.size()[2:3][0]*4, mode='bilinear', align_corners=True)
        return output
...
class DeepLab(nn.Module):
    def __init__(self, class_num, output_stride=16, pretrained=True, bn_momentum=0.1, freeze_bn=False):
        ...
        self.bn_momentum = bn_momentum
        self.FeatExtr = FeatureExtractor(pretrained, bn_momentum, freeze_bn)
        self.FinLay = FinalLayers(class_num, output_stride, pretrained, bn_momentum, freeze_bn)
        if freeze_bn:
            self.freeze_bn()
            print("freeze bacth normalization successfully!")

    def forward(self, input):
        low_level_features = self.FeatExtr(input)
        output = self.FinLay(low_level_features)
        return output
    ...
    def change_o_stride(self, output_stride):
        self.FinLay.change_o_stride(output_stride)
